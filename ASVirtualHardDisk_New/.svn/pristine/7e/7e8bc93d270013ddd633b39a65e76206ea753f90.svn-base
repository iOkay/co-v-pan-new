...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
			NSLog(@"%@",[NSString stringWithFormat:@"%d %s",globalInfo.number_entry,KEntriesinthezipfile] );
		}
	}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	if( ret!=UNZ_OK )
	{
		[self OutputErrorMessage:KFailed];
	}
	...
	do{
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:KError];
			success = NO;
			break;
		}
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:KErrorFile];
			success = NO;
        ...
        NSString * strPath = [[NSString alloc] initWithCString:filename encoding:NSUTF8StringEncoding];
...
//		[strPath release];
		...
		while( fp )
		{
			...
			{
				[self OutputErrorMessage:KReadFaile];
				break;
			}
			else 
				break;				
