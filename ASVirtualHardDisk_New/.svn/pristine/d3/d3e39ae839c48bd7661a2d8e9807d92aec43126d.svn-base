...
- (void)doBytesAvailable
{
	...
	{
#if 1
#define BUFFERSIZE (1024*1024) // the default buffer chunk size
		NSMutableData *buffer = [[NSMutableData alloc] initWithLength:BUFFERSIZE]; // buffer 
		while ( theReadStream && CFReadStreamHasBytesAvailable(theReadStream) ){
			// read 1K and call didReadData
			...
			if ( readSize > 0 ){
				...
				{
					// when delegate is set, pass the current data(up to 1k size) to it.

					NSData *buffer_;
					if ( readSize == BUFFERSIZE ){
						// when the buffer size is same as the max size, doesn't create a NSData instance.
						buffer_ = buffer;
					}
					else{
						// create a NSData instance with readSize.
						buffer_ = [[NSData alloc] initWithBytesNoCopy: (void*)[buffer bytes] length: readSize freeWhenDone:NO];
					}
					[theDelegate onSocket:self didReadData: buffer_ withTag:theCurrentRead->tag];
					if ( readSize < BUFFERSIZE ){
						[buffer_ release];
					}
