...
// ----------------------------------------------------------------------------------------------------------
{
    ...
    NSLog(@"FC:QueuedData:%@",queuedData);
}
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	if ([ commandString length ] > 0)																// If there is a command here
	{
        ...
        NSLog(@"COMMANDArgument:%@",commandString);
...
{	
    ...
    NSLog(@"FC:doStor:%@",arguments);
    NSLog(@"FC:currentFile:%@",currentFile);
    // makes a filepath, using absolute or relative filename
...
{		
	...
	currentDir = [ testDirectory copy ];								// make a copy, and retain release
