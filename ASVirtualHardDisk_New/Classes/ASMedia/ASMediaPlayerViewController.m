...
@interface ASMediaPlayerViewController ()
...
- (IBAction)editBegin
{
    ...
    self.nameFiled.clearsOnBeginEditing = [nameFiled.text isEqualToString:NSLocalizedString(KNameFile, nil)];
...
- (void)useTheRecord
{
    ...
    if ([currentDirectory isEqualToString:@""] == YES) {
        if ([nameFiled.text isEqualToString:NSLocalizedString(KNameFile, nil)]) {
